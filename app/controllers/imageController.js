...
export const getImage = async () => {
  try {
    await connectDB();
    const allSections = await Image.find({ delete_status: "active" }).lean();
...
export const postImage = async (req) => {
  try {
    await connectDB();
    ...
    const saveFile = async (file, filenamePrefix) => {
      ...
      const uploadPath = path.join(process.cwd(), "public", "image", fileName);

      await fs.mkdir(path.dirname(uploadPath), { recursive: true });
      await fs.writeFile(uploadPath, buffer);

      return `/image/${fileName}`; // ✅ public path only
    };
    // Get files
    ...
    let parentSectionId;
    try {
      parentSectionId = parseParentSectionId(getText("product_id"));
    } catch (err) {
      return NextResponse.json(
        { success: false, message: err.message },
        { status: 400 }
    ...
    const imageData = await Image.create({
      image_title: getText("image_title"),
      product_id: parentSectionId,

      sort_order: Number(getText("sort_order")|| 1),
      delete_status: getText("delete_status"),
      status: getText("status"),
      delete_by: Number(getText("delete_by") || 1),
      image_name_1: imagePath1,
    });

    return NextResponse.json(
      {
        message: "Image created and saved successfully",
        success: true,
        imageData,
      },
      { status: 200 }
...
export const putImage = async (req) => {
  try {
    await connectDB();
    const formData = await req.formData();
...
export const deleteImage = async (req) => {
  try {
    ...
    // ✅ Soft delete in DB
    const deleted = await Image.findByIdAndUpdate(
      id,
...
export const restoreImage = async (req) => {
  try {
    await connectDB();
    ...
    const restored = await Image.findByIdAndUpdate(
      id,
...
export const permanentlyDeleteImage = async (req) => {
  try {
    await connectDB();
